...
INSERT INTO fact_province_daily(province_id,case_id,date,total)
    ...
    from temp_fact tf 
    inner join dim_case dc on CONCAT(dc.status_name, '_', dc.status_detail) = tf.case
...
INSERT INTO fact_district_yearly(district_id,case_id,year,total)
    select district_id, dc.id as case_id, to_char(date, 'YYYY-MM') as year, sum(total) as total
    from temp_fact tf 
    inner join dim_case dc on CONCAT(dc.status_name, '_', dc.status_detail) = tf.case
